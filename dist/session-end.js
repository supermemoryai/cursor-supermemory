...
function captureId(sessionId) {
  const digest = createHash3("sha256").update(sessionId).digest("hex");
...
async function main() {
  ...
  if (userTurns.length < 2)
    return;
  let transcript = relevant.map((turn, index) => `${index + 1}. [${turn.role}] ${turn.text}`).join(`
`);
...
${transcript}`;
  ...
  await client.addMemory(content, tags.canonical, {
    type: "conversation",

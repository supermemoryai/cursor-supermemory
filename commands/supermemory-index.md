...
Explore and note:
- Folder structure
- Entry points per the Ecosystem Reference
- Architectural patterns listed for the detected ecosystem
- API routes/endpoints, database models/entities, and data flow
...
After exploring all phases, save findings with **separate** `add_memory` calls — do not combine them into one memory. Prefix each with the area it belongs to. Each call must pass `containerTag` from `<supermemory-context>`:

1. Architecture — modules, entry points, data flow
2. Tech stack — detected ecosystem(s), languages, frameworks, tooling
3. Conventions — naming, file layout, testing, patterns to follow
4. How to run — install, build, test, and run from README/CI/manifests
...
- Skip `node_modules`, `bin`, `obj`, `target`, `vendor`, `__pycache__`, `.gradle`, `build`, `.build`, `_build`, `deps`, generated files, and lockfiles
